...
class CalendarSelectForm(forms.Form):
    ...
    def __init__(self, room_id, *args, **kwargs):
        ...
        self.fields["calendar_choices"].queryset = Calendar.objects.filter(
            room_calendar_id__id=room_id, is_active=True
        )
    ...
    def clean(self):
        ...
        if not get_team_leader.exists():
            ...
            check_user_count = TeamMembers.objects.filter(
                leader_id=get_team_leader.first().id
            ).first()
            room_capacity = Room.objects.get(id=self.room_id)
            if room_capacity.capacity < check_user_count.users.all().count():
                raise ValidationError(
                    "The number of people is more than the capacity of the room"
                )

    def save(self):
        get_leader = TeamLeader.objects.filter(leader_id=self.request.user.id).first()
        add_reserve = Reservation.objects.create(
            room_id_id=self.room_id.__str__(), leader_id=get_leader.id
        )
        add_reserve.save()
